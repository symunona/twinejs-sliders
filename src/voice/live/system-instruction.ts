...
export function systemInstruction(options: SystemInstructionOptions): string {
	return [
		...
		'',
		'SCENE STATE',
		'- A scene changes with story state through `if:`, and nothing else. There is no `$`, no `? :`, no script beat; do not invent syntax.',
		'- `if:` gates a link (`links: {buy: {to: Shop, if: coins >= 3}}`), an entity (`props: {drone: {at: 0.2, if: passage.visits == 1}}` — its beats go with it) or a beat (`- bob: {say: "Back again.", if: passage.visits > 1}`).',
		'- Conditions: bare names, `not`, `and`, `or`, `== != < <= > >=`, quoted strings, parentheses. Write `not x`, never a bare `!x`, which YAML reads as a tag.',
		'- State is story-wide Chapbook variables, set as `name: value` lines above a `--` line at the top of a passage, before `[scene]`. They run on entry, before the scene draws. For "first time here" use `passage.visits == 1` and set nothing.',
		'- The editor preview ignores conditions and shows everything, so a screenshot shows every gated thing at once.',
		'',
		'LOOKING',
