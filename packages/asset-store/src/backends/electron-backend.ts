...
export interface TwineElectronAssetBridge {
	readAssetManifest(): Promise<string | undefined>;
	writeAssetManifest(json: string): Promise<void>;
	readAsset(id: AssetId): Promise<ArrayBuffer | undefined>;
	writeAsset(id: AssetId, data: ArrayBuffer, mime: string): Promise<void>;
...
//      `<story folder>/assets/<id>` plus `assets/manifest.json`.
...
export class ElectronBackend implements StorageBackend {
	...
	constructor(private bridge: TwineElectronAssetBridge) {}
	...
	async readManifest(): Promise<AssetManifest> {
		const json = await this.bridge.readAssetManifest();
	...
	async writeManifest(manifest: AssetManifest): Promise<void> {
		await this.bridge.writeAssetManifest(JSON.stringify(manifest, null, 2));
	}

	async readBlob(id: AssetId): Promise<Blob | undefined> {
		const data = await this.bridge.readAsset(id);

		return data ? new Blob([data]) : undefined;
	}

	async writeBlob(id: AssetId, blob: Blob): Promise<void> {
		await this.bridge.writeAsset(id, await blobBytes(blob), blob.type);
	}

	async deleteBlob(id: AssetId): Promise<void> {
		await this.bridge.deleteAsset(id);
	}
}
